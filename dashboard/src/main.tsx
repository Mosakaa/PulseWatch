...
type AlertRule = { id: number; machine_id: string; metric: string; threshold: number; duration_seconds: number; severity: "info" | "warning" | "critical"; enabled: boolean };
...
function Dashboard({ token, email, onSignOut }: { token: string; email: string; onSignOut: () => void }) {
  ...
  async function updateRule(event: FormEvent<HTMLFormElement>, ruleId: number) {
    ...
    try {
      const rule = await request<AlertRule>(`/alert-rules/${ruleId}`, token, { method: "PUT", body: JSON.stringify({ threshold: Number(values.get("threshold")), duration_seconds: Number(values.get("duration_seconds")), severity: values.get("severity"), enabled: values.get("enabled") === "on" }) });
