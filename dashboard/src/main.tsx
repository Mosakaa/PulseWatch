...
function Dashboard({ token, email, onSignOut }: { token: string; email: string; onSignOut: () => void }) {
  ...
  return <main className="app-shell"><aside className="sidebar"><div><p className="eyebrow">Control room</p><h1>PulseWatch</h1></div><nav><a className="active" href="#overview">Overview</a><a href="#machines">Machines <span>{machines.length}</span></a><a href="#alerts">Alerts <span>{activeAlerts.length}</span></a></nav><div className="account"><span>{email}</span><button onClick={onSignOut}>Sign out</button></div></aside><section className="workspace"><header className="topbar"><div><p className="eyebrow">Infrastructure overview</p><h2>System health</h2></div><span className={error ? "connection issue" : "connection"}>{error || (live ? "Live updates active" : "Connecting live updates")}</span></header><section className="summary" id="overview"><div><span>Monitored machines</span><strong>{machines.length}</strong></div><div><span>Online</span><strong className="healthy">{onlineCount}</strong></div><div><span>Active incidents</span><strong className={activeAlerts.length ? "critical" : "healthy"}>{activeAlerts.length}</strong></div></section><section className="content-grid" id="machines"><div className="machine-list"><div className="section-title"><h3>Machines</h3><span>{onlineCount} online</span></div>{machines.length === 0 ? <p className="empty">Enroll a machine to begin receiving telemetry.</p> : machines.map((machine) => <button key={machine.id} className={`machine-row ${activeMachine?.id === machine.id ? "selected" : ""}`} onClick={() => setActiveMachineId(machine.id)}><i className={`status ${machine.status}`} /><span><b>{machine.name}</b><small>{machine.hostname}</small></span><em>{machine.status}</em></button>)}</div><div className="detail-panel"><div className="section-title"><div><h3>{activeMachine?.name ?? "No machine selected"}</h3><span>{activeMachine?.hostname}</span></div>{activeMachine && <span className={`status-label ${activeMachine.status}`}>{activeMachine.status}</span>}</div><div className="metric-grid">{[{ label: "CPU", metric: "cpu_percent" as const, color: "#157f72" }, { label: "Memory", metric: "memory_percent" as const, color: "#c25423" }, { label: "Disk", metric: "disk_percent" as const, color: "#4b61b8" }].map(({ label, metric, color }) => <article key={metric} className="metric"><div><span>{label}</span><b>{telemetry.length ? `${telemetry.at(-1)?.[metric].toFixed(1)}%` : "--"}</b></div><LineChart data={telemetry} metric={metric} color={color} /></article>)}</div><p className="last-seen">{activeMachine?.last_heartbeat_at ? `Last heartbeat ${new Date(activeMachine.last_heartbeat_at).toLocaleString()}` : "No heartbeat received"}</p></div></section><section className="lower-grid" id="alerts"><div><div className="section-title"><h3>Active alerts</h3><span>{activeAlerts.length} open</span></div><div className="alerts">{activeAlerts.length === 0 ? <p className="empty">No active incidents.</p> : activeAlerts.map((alert) => <article className={`alert ${alert.severity}`} key={alert.id}><span>{alert.severity}</span><div><b>{alert.machine_name}</b><p>{alert.message}</p></div><time>{new Date(alert.created_at).toLocaleTimeString()}</time></article>)}</div></div><form className="enroll-form" onSubmit={enrollMachine}><div className="section-title"><h3>Enroll machine</h3></div><label>Name<input value={machineName} onChange={(event) => setMachineName(event.target.value)} placeholder="api-prod-01" required /></label><label>Hostname<input value={hostname} onChange={(event) => setHostname(event.target.value)} placeholder="api-prod-01.local" required /></label><button className="primary">Create enrollment</button>{enrollment && <div className="enrollment"><b>Agent credentials</b><code>Machine ID: {enrollment.id}</code><code>Token: {enrollment.agent_token}</code></div>}</form></section></section></main>;
}
